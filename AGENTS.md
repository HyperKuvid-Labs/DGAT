...
## DGAT — How to Use This Codebase

You have access to the DGAT dependency graph analysis tool. Use it BEFORE reading files directly.

### Always use DGAT tools first:
- "What does [file] do?" → Use `dgat_context` (not `read`)
- "What depends on [file]?" → Use `dgat_dependents`
- "What would break if I change [file]?" → Use `dgat_impact`
- "Find files related to [concept]" → Use `dgat_search` (not `grep`)
- "Show project architecture" → Use `dgat_blueprint`
- "What's in [directory]?" → Use `dgat_module_summary`

### Only use `read`/`grep` when:
- You need to see actual source code implementation details
- You need to understand syntax, types, or exact function signatures
- DGAT context tells you a file is relevant and you need to read it

### DGAT gives you superpowers:
- **Context**: LLM-generated descriptions of every file + all dependencies/dependents
- **Impact**: Blast radius analysis before any change
- **Search**: Find files by concept, not just text matching
- **Architecture**: Full project blueprint generated from the dependency graph
